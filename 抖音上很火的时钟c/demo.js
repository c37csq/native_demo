...
var secondText = ["一秒", "二秒", "三秒", "四秒", "五秒", "六秒", "七秒", "八秒", "九秒", "十秒",
...
function showTime() {
    ...
    var nowTime = [month, day - 1, week, hour, minute, second];
    ...
    for (var i = 0; i < nowTime.length; i++) {
        var num = nowTime[i];
        list[i][1][num].classList.add('active');
    }
    if (isCircle) {
        ...
        for (var i = 0; i < list.length; i++) {
            for (var j = 0; j < list[i][0].length; j++) {
                var r = (i + 1) * 35 + 50 * i;
                var deg = -360 / list[i][1].length * (j - nowTime[i]);
                var x = r * Math.cos(deg * Math.PI / 180) + widthMid;
                var y = -r * Math.sin(deg * Math.PI / 180) + heightMid;
                var temp = list[i][1][j];
...
function changePosition() {
    for (let i = 0; i < list.length; i++) {
        for (let j = 0; j < list[i][1].length; j++) {
            let x = list[i][1][j].offsetLeft + 'px';
            let y = list[i][1][j].offsetTop + 'px';
            setTimeout(() => {
                list[i][1][j].style.position = "absolute";
                list[i][1][j].style.left = x;
                list[i][1][j].style.top = y;
            }, 50);
        }
    }
}